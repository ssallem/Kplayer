...
1. Windows ZIP을 모두 풀고 `KPLAYER.vbs`를 더블클릭합니다.
...
팝업이 차단되면 공개 사이트의 팝업을 허용하세요. PC 연결 창이 열리지 않으면 `KPLAYER.vbs` 실행 및 버전을 확인하세요. 기존 v1.1.0에는 웹 연결 기능이 없어 업데이트가 필요합니다. 큰 영상은 내 PC에 임시 복사하는 준비 시간이 필요합니다. 원본을 복사 없이 바로 재생하려면 로컬 화면의 파일 열기를 사용할 수 있습니다.
